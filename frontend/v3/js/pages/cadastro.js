  ...
  view.innerHTML = `
    ...
    <div class="grid grid-2" style="align-items:start;">
      <div class="card" id="cadastroFormCard">
        ...
        ${tipo === 'clientes' ? `
        <div id="projetosVinculadosCliente" style="display:none;grid-column:1/-1;margin-top:16px;border-top:1px solid var(--border-soft);padding-top:14px">
          <h4 style="font-size:12.5px;font-weight:700;text-transform:uppercase;letter-spacing:.3px;color:var(--text-faint);margin-bottom:8px">Projetos vinculados</h4>
          <div id="listaProjetosVinculados" style="display:flex;flex-direction:column;gap:6px"></div>
        </div>` : ''}
...
function resetForm(schema){
  ...
  const pv = document.getElementById('projetosVinculadosCliente');
...
function fillForm(schema, item){
  ...
  // Etapa 1 (V3): "Projetos vinculados" — só existe pro cadastro de
  // clientes, e só carrega quando o bloco realmente está na tela
  // (schema.tipo aqui é sempre 'clientes' quando o container existe).
  const painelProjetos = document.getElementById('projetosVinculadosCliente');
  if(painelProjetos){
    painelProjetos.style.display = 'block';
    const listaEl = document.getElementById('listaProjetosVinculados');
    listaEl.innerHTML = `<div class="text-faint" style="font-size:11.5px">Carregando...</div>`;
    apiGet(`/clientes/${item.id}/projetos`).then(projetos => {
      if(!Array.isArray(projetos) || !projetos.length){
        listaEl.innerHTML = `<div class="text-faint" style="font-size:11.5px">Nenhum projeto vinculado ainda.</div>`;
        return;
      }
      listaEl.innerHTML = projetos.map(p => {
        const nomeArquivo = p.orcamento?.nome_arquivo || `Orçamento #${p.orcamento?.id ?? '?'}`;
        const papelLabel = p.papel === 'contratante' ? 'Contratante' : (p.papel === 'titular_conta' ? 'Titular da Conta' : 'Outro');
        const outros = (p.outrosParticipantes || []).length
          ? ` · com ${p.outrosParticipantes.length} outro(s) participante(s)`
          : '';
        return `<div style="padding:6px 10px;background:var(--surface-3);border-radius:8px;font-size:12.5px">
          <b>${nomeArquivo}</b> — ${papelLabel}${outros}
        </div>`;
      }).join('');
    }).catch(() => {
      listaEl.innerHTML = `<div class="text-faint" style="font-size:11.5px">Não foi possível carregar os projetos vinculados.</div>`;
    });
  }
