...
async function pageHistorico(){
  ...
  function renderOrcamentos(){
    ...
    body.innerHTML = `<div class="data-grid">${lista.map(o => {
      ...
      <div class="item-row" style="${isDesatualizado ? 'opacity:0.7;' : ''}">
        ...
        <div class="row-actions">
          ...
          <button class="btn btn-icon btn-ghost" data-fechar-negocio="${o.id}" title="Fechar negócio → Iniciar acompanhamento">${icon('kanban')}</button>
    ...
    body.querySelectorAll('[data-fechar-negocio]').forEach(btn => btn.addEventListener('click', () => {
      const o = orcList.find(x => x.id == btn.getAttribute('data-fechar-negocio'));
      if(o) abrirModalFecharNegocio(o);
    }));
...
// ============================================================
// ETAPA 4 (V3) — gatilho de conversão: liga um orçamento já fechado
// ao pipeline de execução (Etapa 2/3). Idempotente: se o orçamento já
// tem um acompanhamento ativo, não deixa duplicar — manda direto pro
// Kanban com o card já existente em destaque.
// ============================================================
async function abrirModalFecharNegocio(o){
  const acompanhamentos = await apiGetCached('/acompanhamentos');
  const jaExiste = (acompanhamentos || []).find(a => String(a.orcamento_id) === String(o.id) && a.status_geral !== 'cancelado');
  if(jaExiste){
    toast('Este orçamento já tem um acompanhamento em andamento.', 'info');
    window._acompHighlightId = jaExiste.id;
    Router.go('/acompanhamento');
    return;
  }

  const tiposServico = (await apiGetCached('/configuracoes/tipos_servico') || []).filter(t => t.ativo !== false);
  if(!tiposServico.length){
    toast('Nenhum Tipo de Serviço cadastrado ainda. Vá em Configurações → Tipos de Serviço.', 'warning');
    return;
  }

  openModal({
    id: 'modalFecharNegocio',
    title: 'Fechar negócio → Iniciar acompanhamento',
    sub: `${o.cliente_nome || 'Cliente'} · ${o.nome_arquivo || ''}`,
    width: 460,
    bodyHtml: `
      <div class="field">
        <label>Tipo de serviço</label>
        <select class="select" id="fnTipoServico">
          ${tiposServico.map(t => `<option value="${t.id}">${t.nome}</option>`).join('')}
        </select>
        <div class="hint">Define o checklist de etapas que será criado automaticamente pra este projeto.</div>
      </div>
    `,
    footHtml: `
      <button class="btn btn-secondary" id="btnCancelFecharNegocio">Cancelar</button>
      <button class="btn btn-primary" id="btnConfirmFecharNegocio">${icon('kanban')} Iniciar acompanhamento</button>
    `
  });
  refreshIcons();

  document.getElementById('btnCancelFecharNegocio').addEventListener('click', () => closeModal('modalFecharNegocio'));
  document.getElementById('btnConfirmFecharNegocio').addEventListener('click', async () => {
    const tipoServicoId = document.getElementById('fnTipoServico').value;
    const btn = document.getElementById('btnConfirmFecharNegocio');
    btn.disabled = true;
    btn.innerHTML = `${icon('loader')} Iniciando...`;
    refreshIcons();

    const result = await apiPost('/acompanhamentos', { orcamento_id: o.id, tipo_servico_id: parseInt(tipoServicoId) });
    if(result){
      invalidateCache('/acompanhamentos');
      toast('Acompanhamento iniciado!', 'success');
      closeModal('modalFecharNegocio');
      window._acompHighlightId = result.id;
      Router.go('/acompanhamento');
    } else {
      btn.disabled = false;
      btn.innerHTML = `${icon('kanban')} Iniciar acompanhamento`;
      refreshIcons();
    }
  });
}
window.abrirModalFecharNegocio = abrirModalFecharNegocio;
