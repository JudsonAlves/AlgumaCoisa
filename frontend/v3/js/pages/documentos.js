...
// ---------------------------------------------------------------
// DISTRIBUIDORAS — Etapa 6 (V3): mesmo grupo citado pelo GD Pro.
// Cada modelo de documento pode ser marcado como pertencente a uma
// delas, pra depois mostrar "ENEL SP: 4/9 documentos gerados" na
// hora de gerar documentos pra um cliente/projeto.
// ---------------------------------------------------------------
const DISTRIBUIDORAS_LISTA = [
  'CEMIG','ENEL','Equatorial','Energisa','Neoenergia','CPFL','Copel','Celesc','EDP','Light'
];
...
const CATALOGO_VARIAVEIS = [
  ...
  // Etapa 1 (V3): só vem preenchido quando o orçamento tem um
  // participante extra com papel "Titular da Conta/Procuração" (ver
  // bloco "+ Adicionar outro participante ao projeto" no Orçamento).
  { categoria: 'Titular da Conta / Procuração', codigo: 'titular_nome', label: 'Nome do titular da conta (se diferente do contratante)' },
  { categoria: 'Titular da Conta / Procuração', codigo: 'titular_documento', label: 'CPF/CNPJ do titular da conta' },
  { categoria: 'Titular da Conta / Procuração', codigo: 'titular_endereco_completo', label: 'Endereço do titular da conta' },
  { categoria: 'Titular da Conta / Procuração', codigo: 'titular_telefone', label: 'Telefone do titular da conta' },
...
function montarValoresDocumento(data, extras, config){
  ...
  // Etapa 1 (V3): titular da conta = primeiro participante extra com
  // papel 'titular_conta' (ver bloco no Orçamento). Se não houver
  // nenhum, as variáveis titular_* ficam vazias — não quebra nada de
  // quem nunca usa esse bloco.
  const titular = (data.participantesExtra || []).find(p => p.papel === 'titular_conta')?.cliente || null;
  const titularEnderecoCompleto = titular ? [
    [titular.endereco, titular.numero].filter(Boolean).join(', '),
    titular.complemento,
    titular.bairro,
    titular.cep ? `CEP ${titular.cep}` : '',
    [titular.cidade, titular.estado].filter(Boolean).join('/'),
  ].filter(Boolean).join(', ') : '';
  ...
  return {
    ...
    titular_documento: titular ? formatarCpfCnpj(titular.cpf_cnpj) : '',
    titular_endereco_completo: titularEnderecoCompleto,
    titular_telefone: titular?.telefone || '',
      ...
      <div>
        <div class="card">
          ...
          <div class="form-grid1">
            ...
            <div class="field">
              <label>Distribuidora <span class="text-faint" style="font-weight:400">(opcional)</span></label>
              <select class="select" id="docDistribuidora">
                <option value="">— nenhuma / não se aplica —</option>
                ${DISTRIBUIDORAS_LISTA.map(d => `<option value="${d}">${d}</option>`).join('')}
              </select>
            </div>
            <div class="field">
...
function renderListaTemplates(lista){
  ...
  el.innerHTML = `<div class="data-grid">${lista.slice().reverse().map(t => `
    <div class="item-row">
      ...
      <div class="main">
        <div class="title">${t.nome}${t.distribuidora ? ` <span class="badge badge-blue" style="margin-left:6px">${t.distribuidora}</span>` : ''}</div>
      ...
      <div class="row-actions">
        ...
        <button class="btn btn-icon btn-ghost" data-editar="${t.id}" title="Editar nome/distribuidora">${icon('pencil')}</button>
  ...
  el.querySelectorAll('[data-editar]').forEach(btn => btn.addEventListener('click', () => {
    const t = lista.find(x => x.id == btn.getAttribute('data-editar'));
    abrirModalEditarTemplate(t);
  }));
...
async function enviarNovoTemplate(){
  ...
  const distribuidora = document.getElementById('docDistribuidora').value;
  ...
  try{
    ...
    const result = await apiPost('/documentos-templates', { nome, distribuidora, arquivo_base64: base64 });
    if(result){
...
async function abrirModalEditarTemplate(t){
  if(!t) return;
  openModal({
    id: 'modalEditarTemplate',
    title: 'Editar modelo',
    width: 420,
    bodyHtml: `
      <div class="form-grid1">
        <div class="field"><label>Nome do modelo</label><input class="input" id="etNome" value="${(t.nome||'').replace(/"/g,'&quot;')}"></div>
        <div class="field">
          <label>Distribuidora <span class="text-faint" style="font-weight:400">(opcional)</span></label>
          <select class="select" id="etDistribuidora">
            <option value="">— nenhuma / não se aplica —</option>
            ${DISTRIBUIDORAS_LISTA.map(d => `<option value="${d}" ${t.distribuidora===d?'selected':''}>${d}</option>`).join('')}
          </select>
        </div>
      </div>
    `,
    footHtml: `
      <button class="btn btn-secondary" id="btnCancelEditarTemplate">Cancelar</button>
      <button class="btn btn-primary" id="btnSalvarEditarTemplate">${icon('check')} Salvar</button>
    `
  });
  refreshIcons();
  document.getElementById('btnCancelEditarTemplate').addEventListener('click', () => closeModal('modalEditarTemplate'));
  document.getElementById('btnSalvarEditarTemplate').addEventListener('click', async () => {
    const nome = document.getElementById('etNome').value.trim();
    const distribuidora = document.getElementById('etDistribuidora').value;
    if(!nome){ toast('Dê um nome para o modelo', 'warning'); return; }
    const result = await apiPut(`/documentos-templates/${t.id}`, { nome, distribuidora });
    if(result){
      invalidateCache('/documentos-templates');
      toast('Modelo atualizado', 'success');
      closeModal('modalEditarTemplate');
      pageDocumentos();
    }
  });
}
...
async function abrirModalGerarDocumento(templateOverride){
  ...
  // Etapa 1 (V3): se o orçamento tem um participante com papel
  // "Titular da Conta/Procuração" (além do Contratante), sinaliza e
  // pré-seleciona o modelo de Procuração, se existir um cadastrado.
  const temTitularConta = (typeof OS !== 'undefined') && (OS.participantesExtra || []).some(p => p.papel === 'titular_conta');
  const templateProcuracao = temTitularConta
    ? templates.find(t => /procura[cç][aã]o/i.test(t.nome || ''))
    : null;

  const valorTotal = OS.resultado?.totalGeral || 0;

  // Etapa 6 (V3): progresso "ENEL SP: 4/9 documentos gerados" por
  // distribuidora — só faz sentido na tela de escolha geral (não
  // quando já veio um template específico) e com cliente selecionado.
  let progressoDistribuidorasHtml = '';
  if(!templateOverride && OS.cliente){
    const templatesComDistribuidora = templates.filter(t => t.distribuidora);
    if(templatesComDistribuidora.length){
      const gerados = await apiGet(`/clientes/${OS.cliente.id}/documentos`) || [];
      const geradosPorTemplate = new Set(gerados.map(g => String(g.template_id)).filter(Boolean));
      const grupos = {};
      templatesComDistribuidora.forEach(t => {
        (grupos[t.distribuidora] = grupos[t.distribuidora] || []).push(t);
      });
      progressoDistribuidorasHtml = `
      <div class="field span-full" style="background:var(--surface-3);border-radius:8px;padding:10px 12px">
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.3px;color:var(--text-faint);margin-bottom:8px">Progresso por distribuidora</div>
        <div style="display:flex;flex-direction:column;gap:6px">
          ${Object.entries(grupos).map(([dist, ts]) => {
            const feitos = ts.filter(t => geradosPorTemplate.has(String(t.id))).length;
            return `<div style="display:flex;justify-content:space-between;font-size:12px">
              <span>${dist}</span><span class="text-faint">${feitos}/${ts.length} documentos gerados</span>
            </div>`;
          }).join('')}
        </div>
      </div>`;
  ...
  const html = `
    <div class="form-grid1">
      ${temTitularConta ? `
      <div class="field span-full" style="background:var(--surface-3);border-radius:8px;padding:10px 12px;font-size:12px;color:var(--text-faint)">
        ${templateProcuracao
          ? `⚠️ Este projeto tem um Titular da Conta diferente do Contratante — o modelo <b>${templateProcuracao.nome}</b> foi pré-selecionado.`
          : `⚠️ Este projeto tem um Titular da Conta diferente do Contratante — considere gerar também uma Procuração (nenhum modelo com esse nome está cadastrado ainda).`}
      </div>` : ''}
      ${progressoDistribuidorasHtml}
      ${!templateOverride ? `
      <div class="field">
        ...
        <select class="select" id="gdTemplate">
          ${templates.map(t => `<option value="${t.id}" ${templateProcuracao && t.id === templateProcuracao.id ? 'selected' : ''}>${t.nome}</option>`).join('')}
        </select>
  ...
  async function montarValoresAtuais(){
    ...
    const data = {
      ...
      estrutura: OS.estrutura, resultado: OS.resultado, participantesExtra: OS.participantesExtra
    };
      ...
      try{
        ...
        if(OS.cliente && OS.cliente.id){
          try{
            ...
            invalidateCache(`/clientes/${OS.cliente.id}/documentos`);
          } catch(e){
